...
hyundai_M['MA20'] = hyundai_M['Close'].rolling(20).mean()
hyundai_M['MA60'] = hyundai_M['Close'].rolling(60).mean()
...
index = np.arange(len(hyundai_M.index))
dohlc = np.hstack((np.reshape(index, (-1, 1)), hyundai_M))
axes[0].set_title('현대자동차', fontsize=35)

# 이동평균선 그리기
axes[0].plot(index, hyundai_M['MA5'], label='MA5', linewidth=0.7, color='#94EB3E')
axes[0].plot(index, hyundai_M['MA20'], label='MA20', linewidth=0.7, color='#FFC800')
axes[0].plot(index, hyundai_M['MA60'], label='MA60', linewidth=0.7, color='#D27328')
...
# 거래량 차트
axes[1].bar(index, hyundai_M['Volume'], color='green', width=0.9, align='center',label="Volume")
