...
print("-------------- 2 -----------------")
bilboard_songs=bilboard_long[['year','artist','track','time']] #중복데이터 모으기
print(bilboard_songs)
bilboard_songs=bilboard_songs.drop_duplicates() # 중복 제거
print(bilboard_songs)
...
bilboard_ratings=bilboard_long.merge(bilboard_songs,left_on=['year','artist','track','time'],right_on=['year','artist','track','time']) # join 하는 것과 비슷

# left_on과 right_on이 동일하면 on으로 줄여쓸 수 있습니다. 
print("-------------- 3 -----------------")
bilboard_ratings=bilboard_long.merge(bilboard_songs,on=['year','artist','track','time'])
print(bilboard_ratings)
