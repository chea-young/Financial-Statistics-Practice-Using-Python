...
# 판다스에서 누락값 확인방법
import pandas as pd
print(pd.isnul(NaN)) # isnul(): nul이지? 응, nul이야
print(pd.isnul(nan)) # isnul(): nul이지? 응, nul이야
print(pd.isnul(NAN)) # isnul(): nul이지? 응, nul이야
# 판다스에서 누락값이 아닌 것 확인방법
print(pd.notnul(NaN)) # notnul(): nul 아니지? 아니야, nul이야
print(pd.notnul(42)) # notnul(): nul 아니지? 응, nul아니야
print(pd.notnul('mising'))# notnul(): nul 아니지? 응, nul아니야
#%% 2. 누락값이 생기는 이유
